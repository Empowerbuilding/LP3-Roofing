  ...
  return (
    <nav className="sticky top-0 z-50 overflow-hidden shadow-xl" style={{ height: '80px' }}>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
        ...
        {/* Logo — sits on black side */}
        <Link href="/" className="flex-shrink-0 z-10" style={{ width: '160px' }}>
          <Image
        ...
        {/* Nav links + CTA — sits on white side */}
        ...
        {/* Hamburger */}
        <button
          className="md:hidden z-10 p-2 text-gray-800"
        ...
        <div className="md:hidden absolute top-full left-0 right-0 z-50 bg-white border-t border-gray-200 px-6 py-6 space-y-4 shadow-xl">
          {['Services', 'Gallery', 'About', 'Contact'].map(name => (
            <Link
              key={name}
              href={`/${name.toLowerCase()}`}
              className="block font-display text-base font-semibold text-gray-800 hover:text-black tracking-[3px] uppercase"
          ...
          <div className="pt-4 border-t border-gray-200 space-y-3">
            ...
            <Link
              href="/contact"
              className="block font-display text-center py-3 bg-black text-white text-sm font-bold tracking-[2px] uppercase"
              onClick={() => setMobileOpen(false)}
            >
              Free Inspection
            </Link>
